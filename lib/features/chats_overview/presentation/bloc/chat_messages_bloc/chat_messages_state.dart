...
/*
abstract class ChatMessagesDisplayChanged extends ChatMessagesState {
  final List<Widget> messages;
  final DateTime? _lastLoadedDate;
  ChatMessagesDisplayChanged(
    this.messages,
    this._lastLoadedDate,
  );
 ...
 class ChatMessagesNewPageLoaded extends ChatMessagesDisplayChanged {
  ChatMessagesNewPageLoaded({
    required List<Widget> messages,
    required DateTime? lastLoadedDate,
  }) : super(
          messages = messages,
          lastLoadedDate = lastLoadedDate,
        );

  factory ChatMessagesNewPageLoaded.initialPage(List<Message> initMessages) {
    final widgets = _mapMessagesToWidgets(null, initMessages);
    DateTime? newLastLoadedDate;
    if (initMessages.isNotEmpty) newLastLoadedDate = initMessages.first.date;
    return ChatMessagesNewPageLoaded(
        messages: widgets, lastLoadedDate: newLastLoadedDate);
  }

  factory ChatMessagesNewPageLoaded.fromAllreadyLoadedMessages(
    ChatMessagesDisplayChanged lastState,
    List<Message> newMessages,
  ) {
    final newWidgets = _mapMessagesToWidgets(
      lastState._lastLoadedDate,
      newMessages,
    );
    DateTime? newLastLoadedDate;
    if (newMessages.isNotEmpty) newLastLoadedDate = newMessages.first.date;
    return ChatMessagesNewPageLoaded(
      messages: [...newWidgets, ...lastState.messages],
      lastLoadedDate: newLastLoadedDate ?? lastState._lastLoadedDate,
    );
  }
}

class ChatMessagesNewMessageAdded extends ChatMessagesDisplayChanged {
  ChatMessagesNewMessageAdded({
    required List<Widget> messages,
    required DateTime? lastLoadedDate,
  }) : super(
          messages = messages,
  ...
  factory ChatMessagesNewMessageAdded.fromAllreadyLoadedMessages(
    ChatMessagesDisplayChanged lastState,
    List<Message> newMessages,
  ) {
    final newWidgets = _mapMessagesToWidgets(
    ...
    DateTime? newLastLoadedDate;
    if (newMessages.isNotEmpty) newLastLoadedDate = newMessages.first.date;
    return ChatMessagesNewMessageAdded(
      messages: [
        ...lastState.messages,
        ...newWidgets,
      ],
